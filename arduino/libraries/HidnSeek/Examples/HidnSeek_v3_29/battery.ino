...
unsigned int analogReadSorted(byte sensor) {
  analogReference(INTERNAL);
  uint8_t sortedValues[NUM_READS];
  for (uint8_t i = 0; i < NUM_READS; i++) {
    uint8_t value = analogRead(sensor) >> 2;
    uint8_t j;
...
bool batterySense() {
  boolean ledRed = (PORTD & (1 << redLEDpin));

  //analogReference(EXTERNAL);
  if (!ledRed) {
    PORTD |= (1 << redLEDpin);
    delay(30); // RC need 30ms
  }
  // read multiple values and sort them to take the median value. Require 24ms
  batteryValue = analogReadSorted(sensorBatt);
