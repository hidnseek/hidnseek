...
uint8_t MMA8653::_read_register(uint8_t offset) {
	I2c.read((uint8_t)_addr, (uint8_t)offset, (uint8_t)1);
  return I2c.receive();
}

void MMA8653::_write_register(uint8_t b, uint8_t offset) {
  I2c.write((uint8_t)_addr, b, offset);
}
...
void MMA8653::standby() {
  // Set to status reg
  uint8_t reg1 = _read_register((uint8_t)MMA_8653_CTRL_REG1);
  // Reset
  _write_register((uint8_t)MMA_8653_CTRL_REG1, reg1 & ~MMA_8653_CTRL_REG1_VALUE_ACTIVE);
}

void MMA8653::active() {
  // Set to status reg
  uint8_t reg1 = _read_register((uint8_t)MMA_8653_CTRL_REG1); // Reset
  _write_register((uint8_t)MMA_8653_CTRL_REG2, 0x09); // Reset
  _write_register((uint8_t)MMA_8653_CTRL_REG1, reg1 | MMA_8653_CTRL_REG1_VALUE_ACTIVE | (_highres ? 0 : MMA_8653_CTRL_REG1_VALUE_F_READ) | MMA_8653_ODR_6_25);
}
...
void MMA8653::begin(bool highres, uint8_t scale) {
  ...
  // Reset
  _write_register(MMA_8653_CTRL_REG2, MMA_8653_CTRL_REG2_RESET);
  delay(10); // Give it time to do the reset
  standby();

#ifdef _MMA_8653_PORTRAIT_LANDSCAPE
  // Set Portrait/Landscape mode
  _write_register(MMA_8653_PL_CFG, 0x80 | MMA_8653_PL_EN);
#endif

  uint8_t data;
  if (_scale == 4 || _scale == 8)
    data = (_scale == 4) ? MMA_8653_4G_MODE : MMA_8653_8G_MODE;
  else // Default to 2g mode
    data = (uint8_t)MMA_8653_2G_MODE;
  _write_register(MMA_8653_XYZ_DATA_CFG, data);
  active();
}
...
byte MMA8653::update() {
  byte error = I2c.read((uint8_t)_addr, (uint8_t)0x00, (uint8_t)_highres ? 7 : 4);
  if (error == 0) 
  {
    _stat = I2c.receive();
    if(_highres)
    {
      _x = (int16_t)((I2c.receive() << 8) + I2c.receive());
      _y = (int16_t)((I2c.receive() << 8) + I2c.receive());
      _z = (int16_t)((I2c.receive() << 8) + I2c.receive());
#ifdef _MMA_8653_FACTOR
